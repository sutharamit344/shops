        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8 animate-in fade-in duration-700">
          ...
          <div className="relative group w-full md:w-80">
             ...
             <input 
               ...
               className="w-full h-10 bg-white border border-black/[0.08] rounded-lg pl-10 pr-4 text-[13.5px] font-medium outline-none focus:border-[#FF6A00]/40 transition-all shadow-sm placeholder:text-[#0A0A0F]/20"
               value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            {categories.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition-all ${
                  activeCategory === cat 
      ...
      <main className="max-w-7xl mx-auto px-4 py-16 relative z-10">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="h-[380px] bg-white rounded-lg animate-pulse border border-black/[0.05]"></div>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredBlogs.map((blog, idx) => (
                <Link 
                  key={blog.id} 
                  href={`/blog/${blog.slug}`} 
                  className={`group flex flex-col h-full bg-white rounded-lg border border-black/[0.05] overflow-hidden hover:border-[#FF6A00]/40 transition-all duration-300 shadow-sm hover:shadow-xl ${idx === 0 && searchQuery === "" && activeCategory === "All" ? "md:col-span-2 lg:col-span-2 md:flex-row" : ""}`}
                >
                  ...
                  <div className="flex-1 flex flex-col p-6 md:p-8">
                    ...
                    <div className="mt-auto flex items-center justify-between pt-5 border-t border-black/[0.03]">
                       <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-black/[0.02] border border-black/5 flex items-center justify-center relative overflow-hidden shrink-0 shadow-sm">
            ...
            <Card className="p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-10 bg-white shadow-2xl border-none">
               ...
               <form className="flex-1 max-w-lg w-full flex flex-col sm:flex-row gap-3">
                  <input 
                    ...
                    className="flex-1 h-11 bg-white border border-black/[0.08] rounded-lg px-4 text-[13.5px] font-medium outline-none focus:border-[#FF6A00]/40 transition-all shadow-sm"
                  />
          ...
          <div className="text-center py-32 bg-white rounded-lg border border-dashed border-black/[0.1] animate-in fade-in">
             <div className="w-12 h-12 bg-black/[0.02] rounded-lg flex items-center justify-center mx-auto mb-6 text-[#0A0A0F]/10">
                <FileText size={24} />
             </div>

...
import { Plus, Check, X, Tag, Loader2, CircleAlert, Trash2, Edit2, Search, ChevronDown } from "lucide-react";
import {
  getCategories,
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-white rounded-[32px] border border-[#0A0A0F]/[0.06] shadow-md overflow-hidden">
        <div className="p-6 md:p-8 border-b border-[#0A0A0F]/[0.06] flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-50/50">
          <div>
            <h2 className="text-2xl font-bold text-[#0A0A0F] tracking-tight">Sub Category Master</h2>
                ...
                <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-[#0A0A0F]/5 text-[#0A0A0F] rounded-lg flex items-center justify-center group-hover:bg-[#FF6A00] group-hover:text-white transition-all">
                        <CategoryIcon name={sub.name} size={18} />
                      </div>
                      <span className="font-bold text-[#0A0A0F] text-[15px]">{sub.name}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#FF6A00]" />
                      <span className="text-[13px] font-bold text-[#0A0A0F]/75">{sub.parentCategory}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="w-10 h-10 bg-gray-50 text-[#0A0A0F]/60 rounded-lg hover:bg-[#0A0A0F] hover:text-white transition-all flex items-center justify-center border border-black/[0.03]"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
      ...
      <Dialog isOpen={showAddModal} onClose={() => setShowAddModal(false)} maxWidth="max-w-md" showHeader={false}>
        <div className="p-8">
          <h3 className="text-xl font-bold text-[#0A0A0F] mb-1">New Subcategory</h3>
          <form onSubmit={handleAdd} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full h-14 rounded-lg bg-gray-50 border border-[#0A0A0F]/[0.08] px-5 font-bold outline-none focus:border-[#FF6A00] transition-all appearance-none cursor-pointer"
                >
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={isProcessing || !newName.trim()} className="flex-1 h-12 bg-[#FF6A00] text-white rounded-lg font-bold text-[13px] shadow-md hover:bg-[#E85C25] transition-all disabled:opacity-50">{isProcessing ? <Loader2 className="animate-spin mx-auto" size={18} /> : "Initialize"}</button>
            </div>
          </form>
        </div>
      </Dialog>
      ...
      <Dialog isOpen={showEditModal} onClose={() => setShowEditModal(false)} maxWidth="max-w-md" showHeader={false}>
        <div className="p-8">
          ...
          <h3 className="text-xl font-bold text-[#0A0A0F] mb-1">Update Sub category</h3>
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={isProcessing || !editedName.trim()} className="flex-1 h-12 bg-[#0A0A0F] text-white rounded-lg font-bold text-[13px] shadow-md hover:bg-[#333] transition-all disabled:opacity-50">{isProcessing ? <Loader2 className="animate-spin mx-auto" size={18} /> : "Update Subcategory"}</button>
            </div>
          </form>
        </div>
      </Dialog>
      ...
      <Dialog isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} maxWidth="max-w-md" showHeader={false}>
        <div className="p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-[#0A0A0F] mb-1">Remove Subcategory</h3>

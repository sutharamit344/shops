                ...
                <div className="space-y-2 mt-2 pt-4 border-t border-gray-50 animate-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {allClusters
                      ...
                      .map(cluster => (
                        <div key={cluster.id} className="flex flex-col gap-0.5 px-3 py-2 bg-gray-50 text-[#1A1F36] rounded-lg border border-[#1A1F36]/[0.05] group/cluster">
                          <span className="text-[11px] font-bold">{cluster.name}</span>
                          {(cluster.area || cluster.city) && (
                            <span className="text-[9px] text-[#999] font-bold uppercase tracking-tight">{cluster.area}{cluster.area && cluster.city ? ', ' : ''}{cluster.city}</span>
                          )}
                          <button
                            onClick={() => handleRejectCluster(cluster.id)}
